...
- **Aislamiento por usuario, revisado explícitamente (sin fallos encontrados):** cada endpoint de `rutinas.py` que lee/edita/borra remonta la comprobación de dueño hasta `rutinas.usuario_id` (los huecos se protegen protegiendo su rutina; los comodines, protegiendo su hueco) — repetir este mismo patrón al construir `entrenamientos`/`series`. Tampoco hay riesgo de inyección SQL en ningún router: todas las consultas usan el query builder de SQLAlchemy (`select().where(...)`), que siempre parametriza los valores — nunca se concatena texto de la petición dentro de una sentencia SQL. Y mandar campos de más en el body (ej. `usuario_id` en un `POST /rutinas`) no sirve para nada: los esquemas Pydantic no los declaran, y por defecto Pydantic ignora silenciosamente cualquier campo no declarado.
