...
- **Estructura de la app (routers + `auth.py`)**: los endpoints viven en `app/routers/` (un archivo por entidad: `ejercicios.py`, `grupos_musculares.py`, ...), no todos en `main.py`, pensando en que el proyecto va a crecer (rutinas, entrenamientos, series). El usuario hardcodeado (mientras no exista JWT) no se referencia como una constante suelta en cada endpoint, sino a través de una dependencia de FastAPI, `get_usuario_actual_id()` en `app/auth.py` — así, al implementar JWT, solo cambia la implementación de esa función (pasa a leer el usuario del token), sin tocar ningún endpoint.
...
- **`DELETE /ejercicios/{id}` implementa la estrategia de borrado de la sección "Esquema de base de datos"** vía un parámetro de query `?modo=ocultar|definitivo`: sin historial dependiente, borra directamente (sin necesidad de `modo`); con historial, exige `modo` explícito y si no lo recibe devuelve `409` con las dos opciones explicadas en el mensaje. La comprobación de historial (`_tiene_historial()` en `app/routers/ejercicios.py`) hoy siempre devuelve `False` — todavía no existen `series`/`rutina_slots`/`slot_alternativas`, así que no hay nada real que detectar. Hay que completarla con consultas reales en cuanto esas tablas se implementen; hasta entonces, el borrado siempre es directo y la rama `409` no se puede probar de verdad.
